...
class edit_templates {
    ...
    function view_select_template() {
        ...
        $templates_data = $DB->get_records_select('assignsubmission_pdf_tmpl', 'courseid = 0 OR courseid = ?', array($this->courseid) );
    ...
    function view_template_edit_form() {
        ...
        echo '<input type="checkbox" name="sitetemplate"'.$sitetemplate.' id="sitetemplate" /><label for="sitetemplate">'.get_string('sitetemplate', 'assignsubmission_pdf').' </label>';
        echo get_string('sitetemplatehelp','assignsubmission_pdf');
        echo '<br /><br />';
        echo '<input type="submit" name="savetemplate" value="'.get_string('savetemplate','assignsubmission_pdf').'"'.$editdisabled.' />&nbsp;';
        if (($this->templateid > 0) && ($uses == 0)) {
            $deletedisabled = $editdisabled;
        } else {
            $deletedisabled = ' disabled="disabled" ';
        }
        echo '<input type="submit" name="deletetemplate" value="'.get_string('deletetemplate', 'assignsubmission_pdf').'"'.$deletedisabled.'/>';
        echo '<br />';
        echo '<input type="submit" name="duplicatetemplate" value="'.get_string('duplicatetemplate', 'assignsubmission_pdf').'"/>';
        if ($this->templateid > 0) {
            echo '<br />';
            ...
            echo '</select>';
            echo '<input type="submit" name="selectitem" id="selectitem" value="'.get_string('select','assignsubmission_pdf').'" />';
        }
    ...
    function view_item_form() {
        ...
        echo 'else if (oldtype == "date") { settingel.value = "'.get_string('enterformtext','assignsubmission_pdf').'"; Highlight("itemsetting"); } ';
        echo 'UpdatePreview();';
        ...
        echo '<table>';
        echo '<tr><td width="150pt"><label id="itemtype_label" for="itemtype">'.get_string('itemtype','assignsubmission_pdf').': </label></td>';
        echo '<td><select id="itemtype" onchange = "type_changed();" name="itemtype"'.$disabled.'>';
        echo '<option value="shorttext"'.(($item->type == 'shorttext') ? ' selected="selected" ' : '').'>'.get_string('itemshorttext','assignsubmission_pdf').'</option>';
        echo '<option value="text"'.(($item->type == 'text') ? ' selected="selected" ' : '').'>'.get_string('itemtext','assignsubmission_pdf').'</option>';
        echo '<option value="date"'.(($item->type == 'date') ? ' selected="selected" ' : '').'>'.get_string('itemdate','assignsubmission_pdf').'</option>';
        echo '</select></td></tr>';

        echo '<tr><td><label for="itemx" id="itemx_label">'.get_string('itemx','assignsubmission_pdf').': </label></td>';
        echo '<td><input type="text" id="itemx" name="itemx" onChange="Highlight(\'itemx\');" value="'.$item->xpos.'"'.$disabled.' /> '.get_string('clicktosetposition','assignsubmission_pdf').'</td></tr>';
        ...
        echo '<tr><td><label for="itemsetting" id="itemsetting_label">'.get_string('itemsetting','assignsubmission_pdf').': </label></td>';
        echo '<td><input type="text" id="itemsetting" name="itemsetting" onChange="Highlight(\'itemsetting\');" value="'.$item->setting.'"'.$disabled.' /> '.get_string('itemsettingmore','assignsubmission_pdf');
        echo ' <a href="http://www.php.net/date" target="_blank">'.get_string('dateformatlink','assignsubmission_pdf').'</td></tr>';
        echo '<tr><td><label for="itemwidth" id="itemwidth_label" >'.get_string('itemwidth','assignsubmission_pdf').': </label></td>';
        echo '<td><input type="text" id="itemwidth" name="itemwidth" onChange="Highlight(\'itemwidth\');" value="'.$item->width.'"'.$disabled.' /> '.get_string('textonly','assignsubmission_pdf').'</td></tr>';
        echo '</table>';
        echo '<input type="submit" name="saveitem" id="saveitem" value="'.get_string('saveitem','assignsubmission_pdf').'"'.$disabled.'/>&nbsp;';
        echo '<input type="submit" name="cancelitem" id="cancelitem" value="'.get_string('cancel').'"'.$disabled.'/>&nbsp;&nbsp;';
        echo '<input type="submit" name="deleteitem" value="'.get_string('deleteitem','assignsubmission_pdf').'"'.$deletedisabled.'/>';
        echo '</fieldset>';
        echo '</form>';
    ...
    function save_template($id, $name, $sitewide = false) {
        ...
        if ($id == -1) {
            ...
            } elseif ($template->courseid != $this->courseid) {
...
class edit_templates_form extends moodleform {
    function definition() {
        ...
        $mform =& $this->_form;
        $mform->addElement('filepicker', 'preview', get_string('previewinstructions','assignsubmission_pdf'), null, array('accepted_types'=>array('*.pdf')));
